...
const std::vector<CommandSpec>& commands() {
    static const std::vector<CommandSpec> k = {
        ...
        make("quant", "nn quant [options] <model>|compare <a> <b>", "Analyze quantization.",
             ...
             cmd_quant),
        make("sparsity", "nn sparsity [options] <model>",
             "Weight sparsity and prune-candidate hints (inspect only; does not rewrite).",
             {"<model>"},
             {{"threshold", '\0', true, "VALUE", "absolute |w| near-zero threshold"},
              {"channel-frac", '\0', true, "VALUE",
               "weak if channel L1 <= VALUE * max (default 0.01)"}},
